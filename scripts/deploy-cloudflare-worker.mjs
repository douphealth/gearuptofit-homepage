...
const checks = await Promise.all([
  ...
  expect('/shoe-finder/ proxied 200', `https://${zoneName}/shoe-finder/`, (r) => r.status === 200 && r.headers.get('x-proxied-from')),
  expect('/supplement-match/ proxied 200', `https://${zoneName}/supplement-match/`, (r) => r.status === 200 && r.headers.get('x-proxied-from')),
  expect('/fitness-plan/ live no-store', `https://${zoneName}/fitness-plan/`, (r) => r.status === 200 && /no-store|no-cache/.test(r.headers.get('cache-control') || '')),
  expect('/watch-match/ live no-store', `https://${zoneName}/watch-match/`, (r) => r.status === 200 && /no-store|no-cache/.test(r.headers.get('cache-control') || '')),
  expect('/shoe-finder/ live no-store', `https://${zoneName}/shoe-finder/`, (r) => r.status === 200 && /no-store|no-cache/.test(r.headers.get('cache-control') || '')),
  expect('/supplement-match/ live no-store', `https://${zoneName}/supplement-match/`, (r) => r.status === 200 && /no-store|no-cache/.test(r.headers.get('cache-control') || '')),
